<script setup lang="ts">
    ...
    import {
        type Client,
        no_data_handler as mc_no_data_handler,
        loading as mc_loading,
    } from "../../../module/api-fetch/MinecraftVersionManifest";

    const status = ref("pending");
    let version_manifest = reactive({
        release: [] as Client[],
        snapshot: [] as Client[],
        old: [] as Client[],
        fool: [] as Client[],
    ...
    const currentTab = ref(
        localStorage.getItem("X-Download-Manual-CurrentTab") ?? "release"
    );
    ...
    function switch_page(
        ...
        localStorage.setItem("X-Download-Manual-CurrentTab", page);
    }
    ...
    onMounted(async () => {
        ...
        if (isCache) {
            ...
            if (Date.now() > data.cache_expt) {
                await mc_no_data_handler();
            } else {
                const cData = data.data;
                const resp = await mc_loading(cData);
                switch (typeof resp) {
                    case "object":
            ...
            const cData = await mc_no_data_handler();
            switch (typeof cData) {
                case "object":
    ...
    <div
        ...
        fade-l5>
            <input
                type="radio"
                name="radio"
                @click="switch_page(`forge`)" />
            <span class="name">
                <img src="/images/modding/Forge.png" />
                Forge
            </span>
        </label>
        <label class="radio" fade-l6>
            <input
                type="radio"
                name="radio"
                @click="switch_page(`neoforge`)" />
            <span class="name">
                <img src="/images/modding/NeoForge.png" />
                NeoForge
            </span>
        </label>
        <label class="radio" fade-l7>
            <input
                type="radio"
                name="radio"
                @click="switch_page(`fabric`)" />
            <span class="name">
                <img src="/images/modding/Fabric.png" />
                Fabric
            </span>
        </label>
    </div>
...
<style lang="scss" scoped>
    .radio-inputs {
        @apply relative flex flex-wrap rounded-tl-lg rounded-tr-lg bg-white box-border p-1 w-[800px] text-lg;
